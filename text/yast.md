...
**Immediate output.** An utterance does not have to be fully prepared before the actual pronounciation can begin. As soon as some parts of the "underlying" structure are generated, the utterance can be started. Only in some limited situations will it be necessary to postpone the actual utterance.

**Parsing by mirroring.** When confronted with a linguistic utterance, the parser does not try to "reverse engineer" the structure of the utterance, but attempts to mirror the underlying structure by "guessing" and constantly checking whether the guessed structure results in the observed utterance by mirroring the production process internally.
