...
earlyfeatures = ['Plural', 'Kontrolle', 'Vorfeld']

def Artikulator(addto):
...
def Prädikativ(clause, verb):
  ...
  if len(parts) > 1:
    location = parts[1]
    ET.SubElement(predicative, 'ORT', attrib = {'juncture': location})
  # add predicate node
  predicate = ET.SubElement(clause, 'PRÄDIKAT')
...
def Vorgangspassiv(clause, demoted = 'von'):
  ...
  for node in clause.findall('ARGUMENT'):
    ...
    if leaf.get('case') == 'Nominativ':
      ET.SubElement(node, 'VORGANGSPASSIV', attrib = {'case': demoted})
...
def Rezipientenpassiv(clause):
  ...
  for node in clause.findall('ARGUMENT'):
    leaf = list(node.iter())[-1]
    if leaf.get('case') == 'Nominativ':
      ET.SubElement(node, 'REZIPIENTENPASSIV', attrib = {'case': 'von'})
...
def Argument(clause, role = None):
  ...
  if role == 'Prädikativ' and phrase is not None:
    return phrase
...
def Adverbialphrase(clause, connection = None):
  ...
  predicate = clause.find('PRÄDIKAT')
  predicate.addprevious(node)
  return phrase

def Attributivphrase(phrase, connection = None):
  # prepare node
...
def Nomen(phrase, referent):
  ...
  # inflection of noun
  if gender == 'Plural':
    referent = Substantive[referent][gender]
    if case == 'Dativ' and referent[:-1] != 'n':
      referent = referent + 'n'
...
def Possessiv(phrase, referent = None): 
  determiner = phrase.find('DETERMINATIV')
  # ignore when no determiner
  if determiner is None:
    return
...
# this is like PHRASE/SATZ, but only necessary for this one situations
def Link(clause):
...
def Attribut(phrase, adword):
  ...
  # move node after referent when adverb, e.g. 'das Treffen gestern'
  # also for 'selbst'
  if adword in Adverbien:
    phrase.xpath('REFERENT|PRONOMEN|ANAPHER|REFLEXIV')[0].addnext(attribute)
    ET.SubElement(attribute, "ADVERB").text = adword
  # else insert adjective after determiner
  elif adword in Adjektive:
    phrase.find('DETERMINATIV').addnext(attribute)
    # adjective agreement
    case = phrase.get('case')
    gender = phrase.get('gender')
    declension = phrase.get('declension')
    ...
    if referent.text is None:
      ...
      Teilnehmer[name] = gender
      # add to local contenxt for reflexivity
      local = upClause(phrase).get('local')
...
def Kongruenz_old(clause, person, number):
  kind = clause.get('kind')
  # find verb
  finitum = clause.find('PRÄDIKAT')[-1]
  verb = finitum.get('verb')
  ...
  if kind in ['Kontrollsatz', 'Präpositionskontrollsatz']:
    person = clause.get('person')
...
def switchPartizipsatz(clause, leaf):
  ...
  case = leaf.get('case')
  if kind == 'Partizipsatz' and variant in ['Vorzeit', 'Nachzeit']:
    if case == 'Akkusativ':
